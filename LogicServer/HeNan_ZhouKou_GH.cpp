#include "HeNan_ZhouKou_GH.h"
#include "HeNan_ZhouKou_CH.h"
#include "Config.h"
#include "Work.h"
#include "LLog.h"
#include "RoomVip.h"

DECLARE_GAME_HANDLER_CREATOR(MJHeNanZhouKou, HeNan_ZhouKou_GH)

bool HeNan_ZhouKou_GH::startup(Desk *desk)
{
	if (desk)
	{
		shutdown();
...
void HeNan_ZhouKou_GH::shutdown(void)
{
...
void HeNan_ZhouKou_GH::SetDeskPlay()
{
...
void HeNan_ZhouKou_GH::HanderUserChange( User* pUser, LMsgC2SBottomPour* msg )
{
...
void HeNan_ZhouKou_GH::ProcessRobot(Lint pos, User * pUser)
{
	if (pos < 0 || pos > 3)
	{
		return;
...
void HeNan_ZhouKou_GH::HanderUserSpeak( User* pUser, LMsgC2SUserSpeak* msg )
{
...
void HeNan_ZhouKou_GH::HanderUserPlayCard(User* pUser,LMsgC2SUserPlay* msg)
{
	if (m_desk == NULL || pUser == NULL || msg == NULL)
	{
	...
	LLOG_DEBUG("HeNan_ZhouKou_GH::HanderUserPlayCard id:%d pos:%d think_type:%d", pUser->m_userData.m_id, pos, msg->m_thinkInfo.m_type);
	...
	if(msg->m_thinkInfo.m_type == THINK_OPERATOR_OUT)
	{
		if(msg->m_thinkInfo.m_card.size())
		{
			...
			{
				if (m_curGetCard && *m_curGetCard == msg->m_thinkInfo.m_card[0])
				{
					m_curOutCard = m_curGetCard;
					gCB_HeNan_ZhouKou->EraseCard(m_handCard[pos], m_curOutCard);
				...
				{
					LLOG_ERROR( "HanderUserPlayCard user already ting, must take out curgetcard" );
...
// 			ThinkEnd();
	...
	if(unit)
	{
		if(unit->m_type == THINK_OPERATOR_BOMB)
		{
			...
			if (m_curGetCard)
			{
				gCB_HeNan_ZhouKou->EraseCard(m_handCard[pos], m_curGetCard);
		...
		else if(unit->m_type == THINK_OPERATOR_AGANG)
		{
			...
			//
			gCB_HeNan_ZhouKou->EraseCard(m_handCard[pos], unit->m_card[0],4);
			m_desk->BoadCast(sendMsg);
			m_angang[pos] += 1;
		...
		else if(unit->m_type == THINK_OPERATOR_MGANG)
		{
			...
			gCB_HeNan_ZhouKou->EraseCard(m_handCard[pos], unit->m_card[0],1);
...
void HeNan_ZhouKou_GH::HanderUserOperCard(User* pUser,LMsgC2SUserOper* msg)
{
	...
	LLOG_DEBUG("HeNan_ZhouKou_GH::HanderUserOperCard id:%d pos:%d think_type:%d", pUser->m_userData.m_id, pos, msg->m_think.m_type);
...
void HeNan_ZhouKou_GH::OnUserReconnect(User* pUser)
{
	...
	else
	{
		...
		gCB_HeNan_ZhouKou->CheckTing(m_handCard[pos], m_pengCard[pos], m_abombCard[pos], m_gangCard[pos], m_eatCard[pos], m_gameInfo, winCards, nullptr);
	...
	if(nDeskPlayType == DESK_PLAY_GET_CARD && m_needGetCard && pos == nCurPos)
	{
		...
		if (m_curGetCard)
		{
			...
			gCB_HeNan_ZhouKou->EraseCard(tmp,m_curGetCard);
		}
...
void HeNan_ZhouKou_GH::OnGameOver(Lint result,Lint bombpos)
{
...
void HeNan_ZhouKou_GH::DeakCard()
{		
	if (!m_desk || !m_desk->m_vip)
	{
		return;
...
// 	}
	...
	gCB_HeNan_ZhouKou->DealCard(m_handCard[0],m_handCard[1],m_handCard[2],m_handCard[3],m_deskCard, special, m_pt_feng);
	...
	m_deskCard.pop_back();
	gCB_HeNan_ZhouKou->SortCard(m_handCard[m_curPos]);
	...
	for(Lint i = 0 ; i < DESK_USER_COUNT; ++i)
	{
		if(m_desk->m_user[i] != NULL)
		{
			...
			if (i != m_curPos)
			{
				...
				gCB_HeNan_ZhouKou->CheckTing(m_handCard[i], m_pengCard[i], m_abombCard[i], m_gangCard[i], m_eatCard[i], m_gameInfo, winCards, nullptr);
...
void HeNan_ZhouKou_GH::CheckStartPlayCard()
{
...
//摸牌
void HeNan_ZhouKou_GH::SetPlayIng(Lint pos,bool needGetCard,bool gang, bool needThink, bool canhu)
{
	if (m_desk == NULL )
	{
	...
	if (needThink)
	{
		...
		m_thinkInfo[pos].m_thinkData = gCB_HeNan_ZhouKou->CheckGetCardOperator(m_handCard[pos],m_pengCard[pos],m_abombCard[pos],m_gangCard[pos],m_eatCard[pos],m_curGetCard,m_gameInfo);
		// 记录过手杠
...
//	if(m_pt_ting)				改为不管有没有听牌玩法都计算听牌数据
	{
		...
		if (m_tingCard[pos].size() == 0)
		{
			m_thinkInfo[pos].m_tingData = gCB_HeNan_ZhouKou->CheckGetCardTing(m_handCard[pos], m_pengCard[pos], m_abombCard[pos], m_gangCard[pos], m_eatCard[pos], needGetCard?m_curGetCard:NULL, m_gameInfo);
		}
	}
	...
	if (m_needGetCard)
	{
		...
		gCB_HeNan_ZhouKou->SortCard(m_handCard[pos]);
	}
...
// 计算玩家得分
void HeNan_ZhouKou_GH::calcUserScore(Lint result, Lint gold[], Lint& bombCount, Lint winPos[])
{
	//计算杠牌得分
	GetGangSocore(gold);
...
void HeNan_ZhouKou_GH::SetThinkIng()
{
	if (!m_desk)
	{
	...
	for(Lint i = 0 ; i < DESK_USER_COUNT;++i)
	{
		...
		if(i != m_curPos)
		{
			...
			m_thinkInfo[i].m_thinkData = gCB_HeNan_ZhouKou->CheckOutCardOperator(m_handCard[i],m_pengCard[i],m_abombCard[i],m_gangCard[i],m_eatCard[i],m_curOutCard,m_gameInfo);
		}
	}
...
void HeNan_ZhouKou_GH::CheckThink()
{
	bool hu		= false;
	bool Peng	= false;
	bool Chi	= false;
...
void HeNan_ZhouKou_GH::ThinkEnd()
{
	for(int i = 0; i < DESK_USER_COUNT; ++i)
	{
		m_thinkInfo[i].Reset();
...
// 	}
	...
	for (Lint i = 0; i < DESK_USER_COUNT; ++i)
	{
		Lint p = (m_beforePos+i)%DESK_USER_COUNT;
		if (m_thinkRet[p].m_type == THINK_OPERATOR_BOMB)
		{
			m_playerHuInfo[p].type = WIN_SUB_BOMB;//CheckIfBigHu(m_thinkRet[i].m_hu)? WIN_SUB_DBOMB : WIN_SUB_BOMB;
			m_playerHuInfo[p].abombpos = m_beforePos;
			++huCount;

			// 原来HanderUserOperCard  msg->m_think.m_type == THINK_OPERATOR_BOMB 中的消息
			LMsgS2CUserOper send;
			send.m_pos = p;
			send.m_errorCode = 0;
			send.m_think.m_type = m_thinkRet[p].m_type;//msg->m_think;
			send.m_card.m_color = (m_curOutCard == NULL)? 1 : m_curOutCard->m_color;		//临时的 有错误
			send.m_card.m_number =(m_curOutCard == NULL)? 1 : m_curOutCard->m_number;		//临时的 有错误
			send.m_hu = m_thinkRet[p].m_hu;
			send.m_cardCount = m_handCard[p].size();
			for (Lint j = 0; j < send.m_cardCount; ++j)
			{
				CardValue mCard;
				mCard.m_color = m_handCard[p][j]->m_color;
				mCard.m_number = m_handCard[p][j]->m_number;
				send.m_cardValue.push_back(mCard);
			}
			m_desk->BoadCast(send);

			break;		// 有一个人胡 那其他的都不用管了
		}

		if (m_thinkRet[p].m_type == THINK_OPERATOR_MGANG)
			gangPos = p;

		if (m_thinkRet[p].m_type == THINK_OPERATOR_MBU)
			buPos = p;

		if (m_thinkRet[p].m_type == THINK_OPERATOR_PENG)
			pengPos = p;
	}
	...
	if (gangPos != INVAILD_POS)
	{
		...
		m_desk->BoadCast(send);
		gCB_HeNan_ZhouKou->EraseCard(m_handCard[gangPos], m_thinkRet[gangPos].m_card[0],3);
	...
	if (buPos != INVAILD_POS)
	{
		...
		send.m_card.m_number = m_curOutCard->m_number;
		m_desk->BoadCast(send);
		gCB_HeNan_ZhouKou->EraseCard(m_handCard[buPos], m_thinkRet[buPos].m_card[0],3);
	...
	if (pengPos != INVAILD_POS)
	{
		...
		send.m_card.m_number = m_curOutCard->m_number;
		m_desk->BoadCast(send);
		gCB_HeNan_ZhouKou->EraseCard(m_handCard[pengPos], m_curOutCard,2);
		...
		//录像
		std::vector<CardValue> cards(3, m_curOutCard->ToCardValue());
	...
	if (m_beforeType == THINK_OPERATOR_MGANG || m_beforeType == THINK_OPERATOR_MBU)
	{
		...
		{
			if(gCB_HeNan_ZhouKou->IsSame(m_curOutCard,*it))
			{
...
void HeNan_ZhouKou_GH::VideoThink(Lint pos)
{
...
void HeNan_ZhouKou_GH::VideoDoing(Lint op, Lint pos, Lint card_color, Lint card_number)
{
	std::vector<CardValue> cards;
	CardValue card;
	card.m_number = op;
...
bool HeNan_ZhouKou_GH::isGuoHandGang(Card* card,Lint pos)
{
	if(!card || pos < 0 || pos >= 4)
	{
		return false;
...
void HeNan_ZhouKou_GH::InitHunCard()
{
	if(m_gameInfo.m_pt_laizi)//(m_playtype.GametypeGetGhost())
	{
		...
		//Card* card = m_deskCard[L_Rand(0, m_deskCard.size()-1)];
		//Card* card = m_handCard[0][0];
...
void HeNan_ZhouKou_GH::BroadcastBottomPour()
{
...
bool HeNan_ZhouKou_GH::IsPouredOver()
{
	int iDingQueCount = 0;
	for(int i = 0; i < DESK_USER_COUNT; ++i)
	{
...
bool HeNan_ZhouKou_GH::CheckIfBigHu(std::vector<Lint>& vec)
{
	bool isBigHu = false;
...
Lint HeNan_ZhouKou_GH::GetHuBaseSocore(HuInfo* pHuInfo)
{
	Lint base = 0;
	if(pHuInfo == NULL)
		return base;
...
void HeNan_ZhouKou_GH::GetGangSocore(Lint gold[])
{
	ScoreInfo info;
...
void HeNan_ZhouKou_GH::GetPaoSocore(Lint result,Lint gold[], Lint& bombCount, Lint winPos[])
{
	ScoreInfo info;
...
Lint HeNan_ZhouKou_GH::FindPengGangPos(Lint curPos, CardValue card)
{
...
bool HeNan_ZhouKou_GH::isDeskCardEmpty()
{
	int gangCount = 0;
	for(Lint i = 0; i < DESK_USER_COUNT; ++i)
	{
