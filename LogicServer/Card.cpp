...
//是否需要风牌
bool PlayType::gameNeedWind() const
{
	auto iTor = m_playtype.begin();
	for (; iTor != m_playtype.end(); ++iTor)
	{
...
bool PlayType::GametypeGetGhost() const
{
	auto iTor = m_playtype.begin();
	for (; iTor != m_playtype.end(); ++iTor)
	{
		if (*iTor == PT_GHOSTCARDFLOP)
		{
			return true;
		}
	}
...
bool PlayType::GametypeQiangKang() const
{
	auto iTor = m_playtype.begin();
	for (; iTor != m_playtype.end(); ++iTor)
	{
		if (*iTor == PT_KEQIANGGANG)
		{
			return true;
		}
	}
...
bool PlayType::hasPour() const
{
	auto iTor = m_playtype.begin();
	for (; iTor != m_playtype.end(); ++iTor)
	{
		if (*iTor == PT_BOTTOM_POUR)
		{
			return true;
		}
	}
...
bool PlayType::hasHuangzhuang() const
{
	auto iTor = m_playtype.begin();
	for (; iTor != m_playtype.end(); ++iTor)
	{
		if (*iTor == PT_HUANGZHUANGNOTHUANGGANG)
		{
			return true;
		}
	}
...
bool PlayType::checkPlayType(PLAYTYPE playType) const
{
	if (playType == PLAYTYPE::PT_ZK_TING || playType == PLAYTYPE::PT_ZK_DAIFENG
		|| playType == PLAYTYPE::PT_ZK_ZHUANGHUGANG || playType == PLAYTYPE::PT_ZK_ZHUANGDOUBLE
		|| playType == PLAYTYPE::PT_BOTTOM_POUR || playType == PLAYTYPE::PT_QIXIAODUI || playType == PLAYTYPE::PT_NEEDWIND)
	{
		return false;
	}
...
Luint PlayType::checkZhaNiao() const
{
	auto iTor = m_playtype.begin();
...
bool CardManager::CheckAnGang(CardVector& handcard,  CardVector& vec)
{
	if(handcard.size() < 4)
		return false;
	...
	for(Lsize i = 0 ;i+3 < handcard.size(); ++i)
	{
		...
		if(*pcarda== *pcardb)//&& pcarda->m_color != 4)
		{
...
bool CardManager::CheckMGang(CardVector& handcard,CardVector& pengCard,  CardVector& vec)
{
	for(Lsize i = 0 ; i < pengCard.size(); i+=3)
	{
		...
		{
			if(*pcard == *handcard[j])// && pcard->m_color ！= 4)
			{
				vec.push_back(pcard);
				break;
			}
