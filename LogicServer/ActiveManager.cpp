...
void ActiveManager::PXReloadConfig(LMsgLMG2LPXActive* msg)
{
	...
	m_mapGameidToActive.clear();		// 清空配置
...
void ActiveManager::checkAddCard(Lint gameid, LMsgS2CGameOver& over, Desk * const m_desk)
{
	...
	{
		if(over.m_win[i] == WIN_SUB_TINGCARD)	continue;
		/*	if(getCardAndHu(m_mapGameidToActive[gameid], over.m_hu[i], over.m_rewardCardNum[i], over.m_rewardHu[i]))
		{
		m_desk->HanderAddCardCount(i, over.m_rewardCardNum[i], CARDS_OPER_TYPE_ACTIVE, "system_paixing");
		}*/
	}
}
