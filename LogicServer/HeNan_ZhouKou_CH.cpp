...
HeNan_ZhouKou_CH* HeNan_ZhouKou_CH::mInstance = NULL;

HeNan_ZhouKou_CH::HeNan_ZhouKou_CH()
{
}

ThinkVec HeNan_ZhouKou_CH::CheckOutCardOperator(CardVector& handcard,CardVector& pengCard,CardVector& agangCard,CardVector& mgangCard,CardVector& eatCard, Card* outCard,OperateState& gameInfo)
{
	ThinkVec result;
	ThinkUnit unit;
...
ThinkVec HeNan_ZhouKou_CH::CheckGetCardOperator(CardVector& handcard, CardVector& pengCard,CardVector& agangCard,CardVector& mgangCard,CardVector& eatCard,Card* getCard,OperateState& gameInfo)
{
	ThinkVec result;
	ThinkUnit unit;
	gameInfo.bChaTing = false;
...
bool HeNan_ZhouKou_CH::CheckCanHu(CardVector& handcard,CardVector& pengCard,CardVector& agangCard,CardVector& mgangCard,CardVector& eatCard, Card* outCard, OperateState& gameInfo,std::vector<Lint>& vec)
{ 
	vec.clear();

	if(gameInfo.m_playtype.checkPlayType(PT_ZK_DUANMEN) && CheckQueMen(handcard, eatCard, pengCard, agangCard, mgangCard, outCard, gameInfo.m_hCard.size()>0?&gameInfo.m_hCard[0]:nullptr) == 0)
		return false;
...
Lint HeNan_ZhouKou_CH::CheckTing(CardVector& handcard,CardVector& pengCard,CardVector& agangCard,CardVector& mgangCard,CardVector& eatCard, OperateState& gameInfo, std::vector<TingCard>& winCards, const Card* ignore)
{
	Lint ret = -1;
	int retfan = -1;
...
//检测抢杠胡
bool HeNan_ZhouKou_CH::CheckQiangganghu(OperateState& gameInfo)
{
...
bool HeNan_ZhouKou_CH::CheckFourGhost(CardVector& handcard, Card* outCard, bool is_get_card, std::vector<Card>& ghostCard)
{
	int ghostNum = 0;
...
void HeNan_ZhouKou_CH::CheckCanHuNormal(CardVector& handcard,CardVector& pengCard,CardVector& agangCard,CardVector& mgangCard,CardVector& eatCard, Card* outCard, OperateState& gameInfo,std::vector<Lint>& vec)
{
...
void HeNan_ZhouKou_CH::CheckCanHuHun(CardVector& handcard,CardVector& pengCard,CardVector& agangCard,CardVector& mgangCard,CardVector& eatCard, Card* outCard, OperateState& gameInfo,std::vector<Lint>& vec)
{
	// 临时手牌 不包括outCard
	CardVector tmpcards;
	int cardnum[4][9];
	...
	//检查是否可以四鬼胡
// 	if (gameInfo.m_pt_laizi)//(gameInfo.m_playtype.GametypeGetGhost())
// 	{
...
bool HeNan_ZhouKou_CH::matchHu_ShunKe(const CardVector& cards, Lint laiZiCount, bool isneed258, std::vector<Card>* pMatch) const
{
...
Lint HeNan_ZhouKou_CH::checkLaiZi(CardVector& handcard, Card* outCard, bool is_get_card, std::vector<Card>& ghostCard, CardVector& tempCard)
{
	tempCard.clear();
	tempCard = handcard;
	if (outCard)
...
Lint HeNan_ZhouKou_CH::CheckQueMen(CardVector& handcard, CardVector& eatCard, CardVector& pengCard, CardVector& agangcard, CardVector& mgangcard, Card* card, Card* ghost)
{
	CardVector tmpCards;
	tmpCards.insert(tmpCards.end(), handcard.begin(), handcard.end());
	tmpCards.insert(tmpCards.end(), eatCard.begin(), eatCard.end());
	tmpCards.insert(tmpCards.end(), pengCard.begin(), pengCard.end());
	tmpCards.insert(tmpCards.end(), agangcard.begin(), agangcard.end());
	tmpCards.insert(tmpCards.end(), mgangcard.begin(), mgangcard.end());
	...
	{
		if(ghost && *tmpCards[i] == *ghost)
			continue;

		++count[tmpCards[i]->m_color];
	}

	if(count[CARD_COLOR_WAN] == 0)
		++que;
	if(count[CARD_COLOR_TUO] == 0)
		++que;
	if(count[CARD_COLOR_SUO] == 0)
		++que;
	//if(count[CARD_COLOR_ZI] == 0)
	//	++que;
