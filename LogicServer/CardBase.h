...
typedef	boost::function<void (std::map<Lint, std::vector<Lint> >)> FilterTingFunc;
class CardBase
{
public:
	virtual ~CardBase(){};
	...
	//检测是否胡牌	--- 检测胡牌，各自实现自已的。
	virtual bool				CheckCanHu( CardVector& handcard, CardVector& pengCard, CardVector& agangCard, CardVector& mgangCard, CardVector& eatCard, Card* outCard, OperateState& gameInfo, std::vector<Lint>& vec ) = 0;
...
public:
	...
	//特定牌局 发牌
	//virtual void				DealCard2(CardVector& v1,CardVector& v2,CardVector& v3,CardVector& v4,CardVector& v5,const Card* specialCard, bool needwind = false);

	//virtual void				DealCardThr(CardVector& v1,CardVector& v2,CardVector& v3,CardVector& v5, bool needwind);

	//virtual void				DealCardThr2(CardVector& v1,CardVector& v2,CardVector& v3,CardVector& v5,const Card* specialCard, bool needwind);

	//virtual void				DealCardHongZhong(CardVector& v1,CardVector& v2,CardVector& v3,CardVector& v4,CardVector& v5, bool needwind, bool needzhong);

	//virtual void				DealCardHongZhong2(CardVector& v1,CardVector& v2,CardVector& v3,CardVector& v4,CardVector& v5,const Card* specialCard, bool needwind, bool needzhong);

	//配牌使用
	static void			        SwapCardBySpecial(CardVector& cvIn, const Card* specialCard, bool needwind, bool needzhong, bool needflower);
	//void				        SwapCardBySpecial(CardVector& cvIn, const Card* specialCard, bool needwind);

	virtual bool				IsSame(Card* c1,Card* c2);

	//对牌进行排序，万，筒，条，红中
	static void					SortCard(CardVector& vec);
	...
	//检查是否吃
	virtual bool				CheckCanChi(CardVector& handcard, Card* outCard, CardVector& vec);
	//检测暗杠
	virtual bool				CheckAnGang(CardVector& handcard, CardVector& vec);
	//检测暗杠
	virtual bool				CheckAnGangWithHun(CardVector& handcard, CardVector& huncards, std::vector< CardVector >& vec);
	//检测明杠
	virtual bool				CheckMGang(CardVector& handcard, CardVector& pengCard, CardVector& vec);
	//检测明杠
	virtual bool				CheckMGangWithHun(CardVector& handcard, CardVector& huncards, CardVector& pengCard, std::vector< CardVector >& vec);
	...
	// 门前清 不吃 不碰 不明杠
	virtual bool                CheckMenQing(CardVector& eatCard, CardVector& pengCard, CardVector& mgangcard);
	...
	// 检查胡 3n+2牌型带258
	bool                        CheckHoo258( const CardVector& handcard, bool b258 );
	// 检查胡 3n+2牌型
	...
	// 能否成顺刻
	virtual bool						ToBeShunKe( const int card[9], bool bzipai );


	// 检查3n+2牌型 带混的牌handcard里没有混 hunnum表示混的数量
	bool                        CheckHooWithHun( const CardVector& handcard, const Card* outCard, int hunnum );
	// 检查胡 3n+2牌型带258
	bool                        CheckHooWithHun258( const CardVector& handcard, int hunnum, bool b258 );
	// 检查胡 3n+2牌型带258
	bool                        CheckHooWithHun258( int cards[4][9], int hunnum , CardValue& jiangCard );
	...
	int							ToBeShunKeNeedHunEx( const int card[9], bool bzipai, int cardindex[9] );

	bool matchHu_ShunKe(const CardVector& cards, Lint laiZiCount, std::vector<Card>* pMatch = NULL, std::vector<Card>* pJiangVec = NULL,bool jiang258=false);

	virtual bool CheckTing(CardVector& handcard, std::map<Lint, std::vector<Lint>>& tingCardMap,FilterTingFunc filterFunc,bool jiang258=false);
	
	// 判断所有牌是否缺一门
	virtual bool CheckQueYiMen(CardVector& handcard,CardVector& eatCard,CardVector& pengCard,CardVector& agangcard,CardVector& mgangcard,Card* outCard);
public:
	static Card			m_card[CARD_COUNT];	// 初始化之后 值是固定的
	static CardVector	m_cardNormal;		// 万 饼 条
	static CardVector	m_cardZi;			// 东南西北中发白
	static CardVector	m_cardZhong;		// 只有红中
	static CardVector	m_cardFlower;		// 春夏秋冬梅兰竹菊
};
